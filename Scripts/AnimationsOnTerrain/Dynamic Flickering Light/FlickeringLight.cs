...
public class FlickeringLight : MonoBehaviour {
    ...
    void Start ()
    {
        ...
        intensityOrigin = myLight.intensity;
        rangeOrigin = myLight.range;
        ...
        intensityOffset = myLight.intensity * scale;
        rangeOffset = myLight.range * scale;
    ...
    private void IntensityAndRange()
    {
        ...
        {
            intensityDelta = (intensityOrigin + Random.Range(-intensityOffset, intensityOffset) - myLight.intensity) * speed;


            rangeTarget = rangeOrigin + Random.Range(-rangeOffset, rangeOffset);
            rangeDelta = (rangeTarget - myLight.range) * speed;

            setNewTargets = false;
        }

        myLight.intensity += intensityDelta;
        myLight.range += rangeDelta;


        if (Mathf.Abs(myLight.range - rangeTarget) < 5f * scale) //////// CHECK IF TARGET IS NEAR ENOUGH. /////////
            setNewTargets = true;
